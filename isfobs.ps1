...
function sccraj {
    ...
    if($HostList){
        ...
        $servers = zebdau -Domain $targetDomain
    }
    
    
    $servers = ebwczf $servers
    ...
    if (($servers -eq $null) -or ($servers.Count -eq 0)){
        ...
        foreach ($server in $servers){
            ...
            Write-Verbose ("[*] Enumerating server $server ($counter of {0})" -f $($servers.count))
            ...
            if ($server -ne ''){
                ...
                $up = $true
                if(-not $NoPing){
                    $up = srndkx -Server $server
                }
                if($up){
                    
                    $shares = cfmgul -HostName $server
...
$Mod = xitmfs -ModuleName Win32


$FunctionDefinitions = @(
    (arwhnh netapi32 NetShareEnum ([Int]) @([string], [Int], [IntPtr].MakeByRefType(), [Int], [Int32].MakeByRefType(), [Int32].MakeByRefType(), [Int32].MakeByRefType())),
    (arwhnh netapi32 NetApiBufferFree ([Int]) @([IntPtr]))
)


$SHARE_INFO_1 = bchmrt $Mod SHARE_INFO_1 @{
    shi1_netname = tmshom 0 String -MarshalAs @('LPWStr')
    shi1_type = tmshom 1 UInt32
    shi1_remark = tmshom 2 String -MarshalAs @('LPWStr')
}


$Types = $FunctionDefinitions | xsyjug -Module $Mod -Namespace 'Win32'
